...
def get_key_fields(detections, image_shape, key_fields):
...
def cluster_associations_detections(key_detections, key_field_lines, cleaned_detections, image_width, form_threshold=0.5):
    ...
    # keys = {i['levenshtein_closest_string']: [i['top_left'], i['top_right'], i['bottom_right'], i['bottom_left']] for i in key_detections}
    ...
    for key, value in dict_key_word_associations.items():
        ...
        for i, value_x in enumerate(value):
            ...
            #     print(keys[key])
            #     print(value_x)
            #     print(keys[key][-1][-1] - value_x['bottom_left'][-1])
            #     print(key_field_lines[key.replace('.', ' ').replace('  ', ' ')]['lines'] * (value_x['bottom_left'][-1] - value_x['top_left'][-1]) - conf.VERTICAL_DIST_BETWEEN_WORD_AND_KEY_LINES_THRESHOLD)
            # if abs(keys[key][-1][-1] - value_x['bottom_left'][-1]) > (key_field_lines[key.replace('.', ' ').replace('  ', ' ')]['lines'] * (value_x['bottom_left'][-1] - value_x['top_left'][-1]) - conf.VERTICAL_DIST_BETWEEN_WORD_AND_KEY_LINES_THRESHOLD):
            #     continue
            dist = []
...
def get_associated_fields(key_detections, detections, image_height, image_width, key_fields, key_field_lines, form_threshold=0.5):
    ...
    clustered_associations = cluster_associations_detections(key_detections, key_field_lines, cleaned_detections, image_width, form_threshold)
    return clustered_associations