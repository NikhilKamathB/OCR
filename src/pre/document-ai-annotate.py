...
def get_entities(document_text, entities):
    entities_list = []
    for entity in entities:
        ...
        id, type, value, confidence = entity['id'], entity['type'], entity['mentionText'], entity['confidence']
...
def get_key_value_form_fields(document_text, form_fields):
    form_list = []
...
def document_ai(input_directory=None, ouput_directory=None):
    ...
    for item in tqdm([_ for _ in os.listdir(input_directory) if '.jpg' in _.lower() or '.png' in _.lower() or '.jpeg' in _.lower()]):
        ...
        for _, page in enumerate(result['document']['pages']):
            form_fields.append(get_key_value_form_fields(result['document']['text'], page['formFields']))
        ...
        res['entities'] = get_entities(result['document']['text'], result['document']['entities'])
        res['form_field'] = form_fields
        res['tokens'] = tokens
        res['image_width'] = image_width
        res['image_height'] = image_height
        res['raw'] = result
        with open('/home/tyche/Downloads/temp_ip.json', "w") as f: 
            json.dump(res, f)
